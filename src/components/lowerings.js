  ...
  renderLowerings() {
    ...
    return this.props.lowerings.map((lowering, index) => {
      if(index >= (this.state.activePage-1) * maxLoweringsPerPage && index < (this.state.activePage * maxLoweringsPerPage)) {
        ...
        let loweringObservers = (lowering.lowering_additional_meta.lowering_observers)? <span>Observers: {lowering.lowering_additional_meta.lowering_observers.join(", ")}<br/></span> : null
